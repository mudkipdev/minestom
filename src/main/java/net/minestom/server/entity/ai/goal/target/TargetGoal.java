...
public abstract class TargetGoal extends Goal {
    ...
    public TargetGoal(final EntityCreature mob, final boolean mustSee) {
        this(mob, mustSee, false);
    }

    public TargetGoal(final EntityCreature mob, final boolean mustSee, final boolean mustReach) {
        this.mob = mob;
    ...
    public boolean canContinueToUse() {
        ...
        if (target == null) {
            ...
            if (this.mob.getDistanceSquared(target) > within * within) {
                return false;
            } else {
                if (this.mustSee) {
    ...
    protected double getFollowDistance() {
        return this.mob.getAttributeValue(Attribute.FOLLOW_RANGE);
    }
    ...
    protected boolean canAttack(@Nullable final LivingEntity target, final TargetingConditions targetConditions) {
        if (target == null) {
            return false;
        } else if (!targetConditions.test(this.mob, target)) {
            return false;
        } else {
            if (this.mustReach) {
    ...
    private boolean canReach(final LivingEntity target) {
        ...
        if (path == null) {
            ...
            if (last == null) {
                ...
                int xx = last.x - target.getPosition().blockX();
                int zz = last.z - target.getPosition().blockZ();
