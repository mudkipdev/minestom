...
public class Bogged extends Monster implements RangedAttackMob {
    public Bogged() {
        super(EntityType.BOGGED);

        getAttribute(Attribute.MAX_HEALTH).setBaseValue(16.0);
        setHealth(16.0F);
        ...
        getGoalSelector().addGoal(4, new RangedBowAttackGoal(this, 1.0, 70, 15.0F));
        ...
        getTargetSelector().addGoal(3, new NearestAttackableTargetGoal<>(this, Turtle.class, 10, true, false, Bogged::isBabyTurtleOnLand));
    }

    private static boolean isBabyTurtleOnLand(final LivingEntity target) {
        if (!(target instanceof Turtle turtle)) {
            return false;
        }
        return turtle.isBaby() && !isInWater(turtle);
    }
    ...
    public boolean interact(final Player player, final PlayerHand hand) {
        ...
        final BoggedMeta meta = (BoggedMeta) getEntityMeta();
        if (stack.material() == Material.SHEARS && !meta.isSheared()) {
            meta.setSheared(true);
            final ItemEntity drop = new ItemEntity(ItemStack.of(Material.RED_MUSHROOM));
            drop.setInstance(getInstance(), getPosition().add(0.0, getBoundingBox().height(), 0.0));
            getViewersAsAudience().playSound(
                    Sound.sound(SoundEvent.ENTITY_BOGGED_SHEAR, Sound.Source.NEUTRAL, 1.0F, 1.0F), this);
            return true;
        }
    ...
    protected SoundEvent getAmbientSound() {
        return SoundEvent.ENTITY_BOGGED_AMBIENT;
    }

    @Override
    protected SoundEvent getHurtSound(final Damage damage) {
        return SoundEvent.ENTITY_BOGGED_HURT;
    }

    @Override
    protected SoundEvent getDeathSound() {
        return SoundEvent.ENTITY_BOGGED_DEATH;
    }
