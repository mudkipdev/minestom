...
public class ArgumentParser {
    ...
    static {
        ...
        for (ArgumentTypeProvider provider : ServiceLoader.load(ArgumentTypeProvider.class)) {
            provider.registerArguments();
        }
    }

    /**
     * Registers an argument factory usable from {@link #generate(String)}.
     * <p>
     * Platforms are expected to call this for each of their own argument types.
     *
     * @param name    the identifier used inside the format string, case insensitive
     * @param factory creates the argument from its id
     */
    @ApiStatus.Experimental
    public static void register(String name, Function<String, Argument<?>> factory) {
        ARGUMENT_FUNCTION_MAP.put(name.toLowerCase(Locale.ROOT), factory);
    }
